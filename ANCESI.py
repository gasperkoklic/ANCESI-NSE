...
class NeoDenApp:
    ...
    def show_help(self):
        ...
        help_text = (
            ...
            "- Load stock: opens a window that allows the user to select a stock CSV file created by NSE to load"
            ...
            "    4 = big IC mode\n"
            "- Apply all: copies this field value to all components in the list.\n"
            "- All Comment: copies this field value to all components with the same Altium comment in the list.\n"
            "- All Footprint: copies this field value to all components with the same footprint in the list.\n"
            "- Save: saves changes and returns to table. Press Enter as shortcut.\n"
            "- Cancel: discards changes and returns to table.\n\n"
            "STOCK FUNCTION:\nClick on the drop down menu next to the label Stock component and enter the first letters/numbers of a component in the provided stock file (added using the yellow Load Stock button), then click the open drop down menu button (down arrow) next to the input and that will show all matching components presets in the list. Alternatively you can just click the open drop down menu button without entering anything and that will show all items in the stock file. Pick the chosen component preset and then click Apply to apply the parameters of the component to the currently edited component. All comment and All footprint do the same as they do with other parameters, they apply the preset to the edited component and all components with the same Altium comment / footprint and close the editing menu automatically\n\n"
